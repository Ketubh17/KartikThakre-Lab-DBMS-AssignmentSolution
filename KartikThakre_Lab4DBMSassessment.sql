...
create table category(cat_id int primary key auto_increment,
cat_name varchar(20) not null);

create table product(pro_id int primary key auto_increment,
pro_name varchar(20) not null default "Dummy",
pro_desc varchar(60),
cat_id int not null,
foreign key (cat_id) references category(cat_id));

create table supplier_pricing(pricing_id int primary key auto_increment,
pro_id int not null,
...
create table orde(ord_id int primary key auto_increment,
...
create table rating(rat_id int primary key auto_increment,
ord_id int not null,
foreign key (ord_id) references orde(ord_id),
rat_ratstars int not null);

/* 2) Inserted the values into tables */
...
INSERT INTO orde VALUES(109,3000,"2021-0-10",5,3);
...
(select distinct cus_id from orde where ord_amount >= 3000)
group by cus_gender;

/* 4) Display all the orders along with product name ordered by a customer having Customer_Id=2 */

select o.cus_id,p.pro_name from orde o 
...
group by s.supp_id
 having count(s.supp_id)>2;
 
/* 6) Find the least expensive product from each category and print the table with category id, name, product name and price of the product */

 select c.cat_id ,c.cat_name,p.pro_name, min(sp.supp_price) from category as c
 join product as p
...
select  o.ord_id,p.pro_name from orde o
join supplier_pricing sp
on o.pricing_id = sp.pricing_id
join product p
on sp.pro_id =p.pro_id
